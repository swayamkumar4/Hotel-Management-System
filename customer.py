...
class Cust_Win:
    def __init__(self,root):
        ...
        combo_Nationality["value"]=("Indian","American","Britishers")
        combo_Nationality.current(0)
        combo_Nationality.grid(row=7,column=1)
        

        #id proof type number combobox
        ...
        combo_id["value"]=("Adhharcard","Drivinglicense","Passport")
        combo_id.current(0)
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Update","Customer details have benn updated successfully",parent=self.root)
    def mDelete(self):
